...
function openPull(s){
 ...
 for(const c of opts){const b=document.createElement('button');b.type='button';b.className='pull-option';b.innerHTML=`<img src="${c.headshot||''}" alt="" onerror="this.style.visibility='hidden'"><strong>${c.name}</strong><div class="version">${c.versionLabel}</div><div class="stats">${Number(c.stats?.ppg||0).toFixed(1)} PPG · ${Number(c.stats?.rpg||0).toFixed(1)} RPG · ${Number(c.stats?.apg||0).toFixed(1)} APG</div>`;b.onclick=()=>{state.roster[s.id]=c;if(boosted)state.boostedCards.add(c.id);delete state.pendingPulls[s.id];$('pull-modal').classList.add('hidden');renderRoster();if(Object.keys(state.roster).length===12){$('build-status').textContent='Roster complete. Arrange the rotation and set your offense order.';$('save-team').disabled=false}};$('pull-options').appendChild(b)}
...
function playerLine(card,min,opt=3,defender=null,r=null){
 ...
 if(sample&&sample.gp>=2){const rawW=(sample.recent?sample.effective:sample.gp)/((sample.recent?sample.effective:sample.gp)+(sample.recent?8:18)),cap=card.version==='Current'?.40:.24,w=Math.min(cap,rawW);p=p*(1-w)+sample.ppg*w;reb=reb*(1-w)+sample.rpg*w;ast=ast*(1-w)+sample.apg*w}
