...
do $$ begin
  ...
  alter table public.stats_race_daily_scores drop constraint if exists stats_race_daily_score_range_v3;
  alter table public.stats_race_daily_scores drop constraint if exists stats_race_daily_max_range_v2;
  alter table public.stats_race_daily_scores drop constraint if exists stats_race_daily_max_range_v3;
  ...
  alter table public.stats_race_matches drop constraint if exists stats_race_host_score_range_v3;
  alter table public.stats_race_matches drop constraint if exists stats_race_guest_score_range_v2;
  alter table public.stats_race_matches drop constraint if exists stats_race_guest_score_range_v3;
...
alter table public.stats_race_daily_scores add constraint stats_race_daily_score_range_v3 check (score is null or score between 0 and 3000);
alter table public.stats_race_daily_scores add constraint stats_race_daily_max_range_v3 check (max_score between 100 and 3000);
alter table public.stats_race_matches add constraint stats_race_host_score_range_v3 check (host_score is null or host_score between 0 and 3000);
alter table public.stats_race_matches add constraint stats_race_guest_score_range_v3 check (guest_score is null or guest_score between 0 and 3000);
create unique index if not exists stats_race_one_daily_attempt_v3 on public.stats_race_daily_scores(user_id,challenge_date,pool);
...
create or replace function public.stats_race_start_daily_v3(p_challenge_date date,p_pool text,p_player_id text,p_max_score integer)
...
begin
 ...
 insert into public.stats_race_daily_scores(user_id,challenge_date,player_id,pool,max_score) values(uid,p_challenge_date,p_player_id,p_pool,greatest(100,least(3000,p_max_score))) returning * into r; return r;
end $$;

create or replace function public.stats_race_submit_daily_v3(p_challenge_date date,p_pool text,p_score integer,p_max_score integer,p_time_ms integer,p_answers jsonb)
returns public.stats_race_daily_scores language plpgsql security definer set search_path=public as $$
...
create or replace function public.stats_race_daily_leaderboard_v3(p_challenge_date date,p_pool text)
...
create or replace function public.stats_race_create_friend_match_v3(p_friend_username text,p_pool text,p_player_id text,p_max_score integer)
...
create or replace function public.stats_race_find_random_match_v3(p_pool text,p_player_id text,p_max_score integer)
returns public.stats_race_matches language plpgsql security definer set search_path=public as $$
...
create or replace function public.stats_race_submit_match_v3(p_match_id uuid,p_score integer,p_max_score integer,p_time_ms integer)
returns public.stats_race_matches language plpgsql security definer set search_path=public as $$
...
begin
 ...
 s:=greatest(0,least(3000,coalesce(p_score,0))); t:=greatest(0,least(90000,round(extract(epoch from (now()-r.starts_at))*1000)::integer));
...
revoke all on function public.stats_race_start_daily_v3(date,text,text,integer) from public,anon;
revoke all on function public.stats_race_submit_daily_v3(date,text,integer,integer,integer,jsonb) from public,anon;
revoke all on function public.stats_race_create_friend_match_v3(text,text,text,integer) from public,anon;
revoke all on function public.stats_race_find_random_match_v3(text,text,integer) from public,anon;
revoke all on function public.stats_race_submit_match_v3(uuid,integer,integer,integer) from public,anon;
grant execute on function public.stats_race_start_daily_v3(date,text,text,integer) to authenticated;
grant execute on function public.stats_race_submit_daily_v3(date,text,integer,integer,integer,jsonb) to authenticated;
grant execute on function public.stats_race_daily_leaderboard_v3(date,text) to anon,authenticated;
grant execute on function public.stats_race_create_friend_match_v3(text,text,text,integer) to authenticated;
grant execute on function public.stats_race_find_random_match_v3(text,text,integer) to authenticated;
grant execute on function public.stats_race_submit_match_v3(uuid,integer,integer,integer) to authenticated;
create or replace function public.stats_race_version() returns text language sql stable security definer set search_path=public as $$ select 'stats-race-v3'; $$;
grant execute on function public.stats_race_version() to anon,authenticated;
select 'Stats Race v3 pools + multi-answer scoring installed' as installed_feature;
